...
<div class="payment-table pt-5">
    <div class="container">
        <h2>Payment Edit</h2>        
        <form action="{{route('payment.update',$model->id)}}" method="POST" enctype="multipart/form-data">
          @csrf @method('PUT')
          <div>
          ...
          <div class="form-group @error('content') has error @enderror ">
            ...
            <input type="text" name="content" id="" class="form-control" placeholder="Enter Content" value="{{$model->content}}">
            @error('content')
              <small style="color: red; padding-top: 5px" class="help-block">{{$message}}</small>
            @enderror
          </div>  
          ...
          <a class="btn btn-primary ml-1" href="{{route('payment.index')}}">Back</a>
          </div>
        </form>        
      </div>
      </div>
